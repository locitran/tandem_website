...
def on_session(_session_id, _param_state):
    ...
    # Case 2: User-provided input, check validity
        ...
        # session_id_udt = gr.update(value="", label=f"❌ Session {_session_id} not found")
...
def then_session(_param_state, folder, _submit_status):
    """
    ...
    """    
    _session_id = _param_state.get('session_id', None)
    _job_status = _param_state.get('status', None)
    _job_name   = _param_state.get("job_name", None)
